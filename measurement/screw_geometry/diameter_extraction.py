"""
ScrewMetric — Screw Diameter Extraction Utilities
===================================================
Helper functions for extracting the diameter (minor axis) of a detected
...
def extract_diameter_px(mask: np.ndarray) -> float:
    """Extract the screw diameter in pixels from a binary mask.

    Fits a minimum-area rotated rectangle to the largest contour
    and returns the minor axis (diameter direction).

    Args:
        mask: Binary uint8 mask of shape (H, W) where 255 = screw.

    Returns:
        Minor axis length in pixels (screw diameter).
...
def extract_diameter_mm(mask: np.ndarray, scale_mm_per_px: float) -> float:
    """Convert the screw diameter from pixels to millimetres.
    ...
    """
    return extract_diameter_px(mask) * scale_mm_per_px
